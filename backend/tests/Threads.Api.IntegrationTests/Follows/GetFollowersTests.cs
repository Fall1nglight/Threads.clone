...
public class GetFollowersTests : FollowIntegrationTestBase
{
    public GetFollowersTests(CustomWebApplicationFactory factory)
        : base(factory) { }

    [Fact]
    public async Task GetFollowers_ShouldReturnEmptyPage_WhenRequesterHasNoFollowers()
    {
        // Arrange
        ...
        // Act
        var response = await client.GetAsync(FollowTestRoutes.Followers);
    ...
    [Fact]
    public async Task GetFollowers_ShouldReturnOnlyUnblockedIncomingAcceptedUsers()
    {
        // Arrange
        ...
        responseBody.Items[0].Id.Should().Be(seed.IncomingAccepted.Id);
        responseBody.Items[0].Username.Should().Be(seed.IncomingAccepted.UserName);
        responseBody.Items[0].CreatedAtUtc.Should().Be(IntegrationTestData.BaseTime.AddMinutes(3));
    }

    [Fact]
    public async Task GetFollowers_ShouldReturnNextPageInDescendingOrder()
    {
        // Arrange
        var scenarioSeeder = CreateFollowRelationshipScenarioSeeder();
        var seed = await scenarioSeeder.SeedBulkAsync(FollowListKind.Followers, 5);
        var client = await CreateAuthenticatedClientAsync(seed.Requester);

        // Act
        var firstResponse = await client.GetAsync($"{FollowTestRoutes.Followers}?pageSize=2");
        var firstPage = await ReadJsonAsync<PagedResponse<FollowUserDto>>(firstResponse);
        var secondResponse = await client.GetAsync(
            $"{FollowTestRoutes.Followers}?pageSize=2&cursor={Uri.EscapeDataString(firstPage.Cursor!)}"
        );
